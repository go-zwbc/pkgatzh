...
func NewT位置信息() *T位置信息 {
	...
	osmustexist.ROOT(absPath)                           // Ensure source DIR exists, applicable in dev/test scenarios (确保源码目录存在，本包仅适用于开发/测试环境)
...
func getPackage(absPath string, pkgName string) *packages.Package {
	osmustexist.ROOT(absPath) // Ensure source DIR exists, applicable in dev/test scenarios (确保源码目录存在，本包仅适用于开发/测试环境)
	cfg := &packages.Config{
		Mode:  packages.NeedName | packages.NeedModule, // Request package name and module info (请求包名和模块信息)
		Dir:   absPath,                                 // Set working path (设置工作路径)
